...
SAMPLE      read_struct_examples(char *file, STRUCT_LEARN_PARM *sparm)
{
  ...
  EXAMPLE  *examples;
  int     n = 10000;       /* number of examples */
  ...
  int frames_len = 100;
  int i = 0, n_frames = 0, n_examples = 0, int_label;
  char *fid, *label, *feature, *c;
  char prev[50];
  double **fbanks = (double **)my_malloc(sizeof(double *)*frames_len);
  double *fbank;
  int *sequence = (int *)my_malloc(sizeof(int)*frames_len);
  PATTERN *x;
  LABEL *y;
    ...
    *c = '\0';
    ...
    if(strcmp(prev, fid) != 0){
      if(n_frames > 0){
        ...
        x = (PATTERN *)malloc(sizeof(PATTERN));
        x->fbanks = fbanks;
        x->length = n_frames;
        ...
        n_frames = 0;
        frames_len = 100;
        fbanks = (double **)my_malloc(sizeof(double *)*frames_len);
    ...
    if(n_frames >= frames_len){
      frames_len *= 2;
      fbanks = (double **)realloc(fbanks, sizeof(double *)*frames_len);
      sequence = (int *)realloc(sequence, sizeof(int)*frames_len);
    }

    fbanks[n_frames] = fbank;
    sequence[n_frames] = int_label;
    n_frames++;
  }
  free(line);
  ...
  fclose(fp);
  n = n_examples;
  examples = realloc(examples, sizeof(EXAMPLE)*n);
				    ...
				    STRUCT_LEARN_PARM *sparm)
{
  ...
  y.sequence[y.length-1] = max_j;
  for(t=y.length-2;t>=0;t--){
    y.sequence[t] = labels[t][y.sequence[t+1]];
  }
...
double      loss(LABEL y, LABEL ybar, STRUCT_LEARN_PARM *sparm)
{
  ...
  for(i=0;i<y.length;i++){
    if(y.sequence[i] != ybar.sequence[i]){
      err += 1.0;
    }
  }
  err /= (double)y.length;
